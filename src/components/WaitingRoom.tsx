...
export default function WaitingRoom({ gameInfo, setGameInfo}: IWaitingRoomProps): JSX.Element {
    console.log('game info', gameInfo)
    useEffect(() => {
        socket.on('new player joined room', (roomNumber, hostUsername, hostGeneratedID, hostSocketID, players) => {
            setGameInfo({
    ...
    return (
        <>
            ...
            <h2>Players:</h2>
            {gameInfo.players.map((player, i) => {return(
                <p key={i}>{player.username}</p>
            )})}
            </>
            }
        </>
